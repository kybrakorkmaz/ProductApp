...
func (productRepository *ProductRepository) GetAllProducts() []domain.Product {
	...
	if err != nil {
		log.Error("ProductRepository.GetAllProducts error = %v", err)
...
func (productRepository *ProductRepository) AddProduct(product domain.Product) error {
	...
	if err != nil {
		log.Error("ProductRepository.AddProduct error = %v", err)
		return err
	}
	log.Info("ProductRepository.AddProduct success = %v", addNewProduct)
	return nil
}
...
func (productRepository *ProductRepository) GetById(productId int64) (domain.Product, error) {
	...
	getBIdSql := "SELECT * FROM products WHERE id = $1"

	queryRow := productRepository.dbPool.QueryRow(ctx, getBIdSql, productId)
...
func (productRepository *ProductRepository) DeleteByID(productId int64) error {
	...
	if getErr != nil {
		return errors.New("ProductRepository.DeleteByID Product Not Found")
	}
	...
	log.Info("ProductRepository.DeleteByID success = v%", productId)
	return nil
}
func (productRepository *ProductRepository) UpdatePrice(productId int64, newPrice float32) error {
	...
	log.Info("ProductRepository.UpdatePrice success = %v", productId)
	return nil
}
